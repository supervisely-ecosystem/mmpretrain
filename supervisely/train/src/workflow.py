    ...
    try:
        if project_info.type != sly.ProjectType.IMAGES.__str__():
            ...
            project_version_id = api.project.version.create(
                ...
                f"This backup was created automatically by Supervisely before the Train MMClassification task with ID: {api.task_id}",
            )
    except Exception as e:
        sly.logger.warninging(f"Failed to create a project version: {repr(e)}")
        project_version_id = None
...
def workflow_output(api: sly.Api, mmdet_generated_metadata: dict, state: dict):
    try:
